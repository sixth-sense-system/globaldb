...
HDR_BUCKETS = [r"start now", r"pilot set", r"how we integrate this", r"top[- ]?now", r"next steps", r"implementation plan"]

def sha10(s): return hashlib.sha1(s.encode("utf-8")).hexdigest()[:10]
...
def normalize_row(raw, src):
    ...
    _id=sha10(raw+src)
    return dict(id=_id, title=title, theme=theme, type=typ, priority=pr, why="", how="", source=src, created_at=datetime.date.today().isoformat())
...
def main():
    ...
    if not jsonl.exists(): sys.exit("extracted.jsonl missing")
    ...
    if not mdp.exists(): mdp.write_text('# ENERQIS Backlog\n',encoding='utf-8')
    ...
    with csvp.open('a',newline='',encoding='utf-8') as f:
        w=csv.DictWriter(f, fieldnames=['id','title','theme','type','priority','why','how','source','created_at'])
        if write_hdr: w.writeheader()
        for e in entries: w.writerow(e)

    # Miss audit
    if misses:
        miss_p.write_text("# Miss candidates (review and convert to CAPTURE if needed)\n" + "\n".join(f"- {m}" for m in misses), encoding='utf-8')

    # STRICT block if miss file exists
    strict_cfg = False
    try:
        # read extractor policy for strict flags
        pol = (root.parent.parent.parent.parent / "00_repo/.cbr/policies/iis_extractors.yaml").read_text(encoding='utf-8')
        strict_cfg = ("strict: true" in pol.lower()) and ("miss_audit_required: true" in pol.lower())
    except Exception:
        pass
