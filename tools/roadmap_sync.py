...
def write_scorecard(csv_path, md_path, now, nxt, later):
    with open(csv_path,'w',newline='',encoding='utf-8') as f:
        w=csv.writer(f); w.writerow(['bucket','id','title','theme','priority','owner','eta'])
        for b,arr in [('Now',now),('Next',nxt),('Later',later)]:
            for it in arr: w.writerow([b,it.get('id',''),it.get('title',''),it.get('theme',''),it.get('priority',''),'ENERQIS','TBD'])
    lines=['# Living Scorecard — '+datetime.date.today().isoformat(),'']
    for name,arr in [('Now',now),('Next',nxt),('Later',later)]:
        lines.append('## '+name)
        for it in arr: lines.append(f"- **{it.get('title','')}** — [{it.get('theme','')}] {it.get('priority','')} (id:{it.get('id','')})")
        lines.append('')
    pathlib.Path(md_path).write_text('\n'.join(lines),encoding='utf-8')

def write_mission(path, now):
    lines=['# Mission Control — '+datetime.date.today().isoformat(),'','This is your daily home base.','## NOW (do in order)']
    for i,it in enumerate(now,1):
        lines += [f"{i}. **{it.get('title','')}**  _[{it.get('theme','')}] {it.get('priority','')}_",
...
def main():
    ...
    later=[it for it in items if (it.get('priority','') or '').upper() not in ('P0','P1')]

    write_scorecard(a.scoreCsv,a.scoreMd,now,nxt,later)
    write_mission(a.mission,now)
    update_dev_roadmap(a.devRoadmap)
    print('Roadmap Sync wrote living_scorecard.*, mission_control.md, and updated development_roadmap.md.')
