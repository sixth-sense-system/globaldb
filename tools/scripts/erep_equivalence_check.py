...
def main():
    ...
    if not policy:
        print("Policy not found", file=sys.stderr)
        sys.exit(2)
    ...
    (reports_dir / "erep_equivalence_filelist.json").write_text(
        json.dumps(sorted(filelist), indent=2), encoding="utf-8"
    )
    ...
    if "G-EREP-EQUIV" in gates:
        ...
        cache_path = root / policy.get("stabilization", {}).get(
            "write_cache_file", "00_repo/.cbr/reports/erep_write_cache.json"
        )
        cache_ok = cache_path.exists()
        if not cache_ok:
            print(
                "EQUIV gate: write cache missing (expected at %s)" % cache_path,
                file=sys.stderr,
            )
            sys.exit(2)
    ...
    print(
        "EREP Equivalence Guard completed (soft). Ensure your EREP action writes the 'after_full' compliance report to tighten this gate."
    )
    sys.exit(0)
