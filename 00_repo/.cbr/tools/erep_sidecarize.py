...
def erqmeta_for(path:str, payload_hash:str):
    return {
      "doc_type": "machine" if "/machine/" in path.replace("\\","/") or path.startswith("00_repo/.cbr") else "control",
      "spec_version": "2.4",
      ...
      "integrity": {"algo":"sha256","canonical_scope":"payload","value": payload_hash}
    }
...
def main():
    ...
    for p in root.rglob("*"):
        ...
        if any(rel.endswith(ext) for ext in (".json",".yaml",".yml",".md",".txt",".csv",".log",".cbot",".ini",".toml")):
